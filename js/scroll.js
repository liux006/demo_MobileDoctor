function barScroll(oBar,barParent,oList,oFresh,who){
	...
	oList.onmousedown=function(ev){
		...
		var ev=ev||event;
		var disY=ev.pageY,listTop=oList.offsetTop;
		var preY=ev.pageY,speedY=0;
		var onoff=true;
		...
		document.onmousemove=function(ev){
			var ev=ev||event;
			speedY=ev.pageY-preY; preY=ev.pageY;
			
			if(oList.offsetTop>=0){
				if(onoff){onoff=false;disY=ev.pageY;}
				...
				oList.style.top=(ev.pageY-disY)/2.5+'px'; barStyle();
			}else if(oList.offsetTop<=maxTop){
				if(onoff){onoff=false;disY=ev.pageY}
				oList.style.top=(ev.pageY-disY)/2.5+maxTop+'px'; barStyle();
			}else{  oList.style.top=ev.pageY-disY+listTop+'px'; barStyle();  }
		}
		document.onmouseup=function(){
			document.onmousemove=null;document.onmouseup=null;if(_this.releaseCapture){_this.releaseCapture();} oList.style.cursor='';
